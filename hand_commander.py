...
class HandTracker3DRenderer:
    ...
    def draw_hand(self, hand, i):
        ...
        for i,a_b in enumerate(LINES_HAND):
            a, b = a_b
            if b+1 < 21:  # Ensure that 'b' is not the last landmark
                v1 = points[a] - points[b]
                v2 = points[b+1] - points[b]
                angle = angle_between(v1, v2)
                print(f"Angle at joint {b}: {angle:.2f} degrees")
        ...
        for finger in Fingers:
            a = LINES_HAND[finger.value-1][0]
            b = LINES_HAND[finger.value-1][1]
            if b + 1 < 21:  # Ensure that 'b' is not the last landmark
                v1 = points[a] - points[b]
                v2 = points[b+1] - points[b]
                angle = angle_between(v1, v2)
                print(f"{finger.name} (landmarks {a}-{b}): {angle:.2f} degrees")
                angle_list.append(angle)
        self.send_floats_via_serial(angle_list , 0x20 , )
    ...
    def send_floats_via_serial(self, floats, device_id):
        """
        Send a list of floats via a serial port.

        Parameters:
        - floats: A list of 6 floats.
        - device_id: An integer (0-255) representing the device ID.
        - serial_port: An opened pyserial Serial object.
        """
        # Convert the list of floats to a byte array
        float_bytes = struct.pack('6f', *floats)

        # Create a 200-byte header and a 199-byte tail
        header = b'\x00' * 200
        tail = b'\x00' * 199

        # Pack everything together: header + device ID + float bytes + tail
        packet = header + struct.pack('B', device_id) + float_bytes + tail
